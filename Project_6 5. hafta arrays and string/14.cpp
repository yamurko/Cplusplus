...
void compareCode(ifstream& inFile, ofstream& outFile, int list[], int length)
{
	...
	for (int i = 0; i < length; i++)
	{
		cout << setw(6) << list[i] << setw(14) << listOne[i] << endl;
		outFile<< setw(6) << list[i] << setw(14) << listOne[i] << endl;
	}
	
	if (codeOk == false)
	{
		cout << "Digit code and copy code are not same!" << endl;
		outFile << "Digit code and copy code are not same!" << endl;
	}

	else
	{
		cout << "Digit code and copy code are same." << endl;
		outFile << "Digit code and copy code are same." << endl;
	}
}